		...
		<div style="margin: 0 auto; width: 999px;">
			<dl class="Order_information">
				...
				<dd style="line-height: 26px;">
					...
					订单金额：${orderInfo.order.money} <br /> 支付状态：
					<c:if test="${orderInfo.order.paystate==0}">
						<font color="red">未支付</font>&nbsp;&nbsp;|&nbsp;
						<a
							href="${ pageContext.request.contextPath }/order/delorder?id=${orderInfo.order.id}">
							<img
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					 <a
							href="${ pageContext.request.contextPath }/order/payorder?id=${orderInfo.order.id}">
							<img
					...
					<br /> 所属用户：${user.uname } <br />
					收货地址：${orderInfo.order.receiverinfo} <br /> 支付方式：在线支付 <br />

				</dd>
			</dl>

			<table width="999" border="0" cellpadding="0" cellspacing="1"
				...
				<c:forEach items="${orderInfo.map}" var="entry">
					<tr>
						<td align="center" valign="middle" bgcolor="#FFFFFF"><img
							src="${ pageContext.request.contextPath }${entry.key.imgurl}"
